...
Los **tipos fundamentales** son los bloques básicos para construir tipos y estructuras de datos más complejas como:
...
Un **objeto** es un conjunto de bits interpretados según un tipo.

⚠️ **Las variables de tipos fundamentales NO se inicializan por defecto**
...
(la inicialización de grandes bloques de memoria puede ser costosa).

👉 **Pero**: casi siempre debes inicializar las variables al declararlas para evitar errores.

---

## Panorama rápido de tipos de datos fundamentales
...
* Entero más pequeño
* Tamaño definido: **1 byte** (normalmente 8 bits)
* En x86/x86_64 suele ser *signed*: valores en `[-128, 127]`

```cpp
char c = 'A';  // literal de carácter
...
double d3 = 512'232'697'499.052; // separador de dígitos
```

---

## Representación numérica común
...
* `a ⊕ b` → devuelve el resultado
* `a ⊕= b` → guarda el resultado en `a`
...
Devuelven un tipo de comparación:
...
`k < 3` **no se evalúa** porque el resultado ya se conoce.

---

## Tamaños en memoria

Todos los tamaños son múltiplos de `sizeof(char)`.
...
⚠️ **Los tamaños dependen de la plataforma**
...
## Tipos flotantes de tamaño fijo – C++23
...
Permite conocer límites y propiedades de los tipos numéricos.

---

## Narrowing (reducción de tipo)

Conversión de un tipo que puede representar **más valores** a uno que puede representar **menos**.

* Puede perder información
* Generalmente **no genera advertencia**
* Fuente común de bugs
...
## Inicialización con llaves – C++11
...
👉 **Usa llaves para evitar conversiones silenciosas**
...
## Ejemplo con impresión de bits (8 bits)
...
⚠️ Desplazar más bits de los que tiene el tipo es **comportamiento indefinido**.
...
Aquí va **la sección CORREGIDA**, **en español**, **en Markdown**, **sin meter cosas nuevas**, **con los símbolos bien puestos**, **pero sin explicación excesiva**.
Es material **para apuntes**, no para discurso.

Puedes copiar/pegar tal cual.

---

## Conversiones y promociones aritméticas

En C++ existen reglas (heredadas de C) para decidir:

- el **tipo común** de los operandos
- el **tipo del resultado** de una operación binaria
...
Donde `⊕` representa una operación aritmética (`+`, `-`, `*`, `/`, `%`).
...
long double ⊕ cualquier tipo   → long double
double ⊕ float                → double
...
Primero ocurre **promoción de enteros**:

- Todo tipo más pequeño que `int`
  se promueve a:
  - `int` o
  - `unsigned int`
  (según cuál pueda representar todos los valores)

Después ocurre **conversión de enteros** si los tipos siguen siendo distintos.

---

### Reglas de conversión entre enteros

#### Ambos con signo (signed)

```

tipo más pequeño → tipo más grande
...
#### Mezcla: signed ⊕ unsigned

1. Si ambos tienen el **mismo tamaño**:
```
...
- y pueden causar errores difíciles de detectar
...
Esto previene errores difíciles de detectar en tiempo de ejecución.
